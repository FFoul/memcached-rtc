...
static int format_load_trace_path(char *buffer, size_t size,
                                  const char *data_dir, int num_operations) {
  return snprintf(buffer, size, "%s/load_%d.csv", data_dir, num_operations);
}

static int format_run_trace_path(char *buffer, size_t size,
                                 const char *data_dir, int num_operations,
                                 int read_ratio, int worker_id) {
  return snprintf(buffer, size, "%s/run_%d_%d_%d.csv", data_dir, num_operations,
                  read_ratio, worker_id);
}
...
static void rtc_note_set_cmd(conn *connection, item *it) {
  ...
  connection->thread->stats.slab_stats[cls].set_cmds++;
...
static bool rtc_read_exists(conn *connection, uint64_t key) {
  ...
  if (hit) {
    connection->thread->stats.slab_stats[cls].get_hits++;
  } else {
...
static void *run_worker_main(void *arg) {
  ...
  worker->rc = replay_run_trace(&connection, trace_path, worker->num_operations,
                                &worker->result);
  if (worker->rc != 0) {
    worker->rc = 1;
    return NULL;
