...
int Worker::ProcessLocalRead(WorkRequest* wr) {
  ...
  if (likely(IsLocal(wr->addr))) {
    // Set access category as LOCAL (0)
    if (wr->access_start_time > 0) {
      wr->access_category = 0;  // LOCAL
    }
    ...
    for (GAddr i = start_blk; i < end;) {
      ...
      if (unlikely(s == DIR_DIRTY)) {
        ...
        // printf("local_request_cache: Created child lwr, parent wr->access_start_time=%ld, lwr->access_start_time=%ld\n",
        //        wr->access_start_time, lwr->access_start_time);
        lwr->counter = 0;
    ...
    if (unlikely(wr->counter)) {
      ...
      // Record cache check end time for local access (all blocks are local and available)
    ...
    // Set access category based on cache hit/miss for remote access
    if (wr->access_start_time > 0) {
      if (wr->is_cache_hit_) {
        wr->access_category = 1;  // CACHE_HIT
      } else {
        wr->access_category = 2;  // CACHE_MISS
      }
    }
  ...
  if (likely(IsLocal(wr->addr))) {
    ...
    for (GAddr i = start_blk; i < end;) {
      ...
      if (state == DIR_DIRTY || state == DIR_SHARED) {
        ...
        if(state == DIR_SHARED) access_type = local_write_shared;
        else access_type = local_write_dirty;
        ...
        lwr->invalidate_type = (state == DIR_DIRTY) ? 3 : 1; // FETCH_AND_INVALIDATE=3, INVALIDATE=1
        ...
        access_type = local_write_unshared;
#ifdef GFUNC_SUPPORT
        if (wr->flag & GFUNC) {
          epicAssert(wr->gfunc);
    ...
    if (wr->counter) {
      ...
      // Record cache check end time for local write (all blocks are local and available)
      if (wr->access_start_time > 0 && wr->cache_check_end_time == 0) {
        wr->cache_check_end_time = get_time();
      }
      wr->unlock();
    ...
    access_type = remote_write_unshared; // temporary value, it can be changed in ProcessPendingWrite based on message
