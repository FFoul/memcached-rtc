...
#endif
// definitions for added static members
int GAllocFactory::total_servers = 1;
int GAllocFactory::current_server_id = 0;
Worker* GAllocFactory::local_worker = nullptr;
#ifdef GFUNC_SUPPORT
...
int GAlloc::Read(const GAddr addr, const Size offset, void* buf,
  ...
  // Record access start time
  wr.access_start_time = get_time();

  if (wh->SendRequest(&wr)) {
    epicLog(LOG_WARNING, "read failed");
    return 0;
  } else {
    // Record access end time and collect timing stats
    wr.access_end_time = get_time();
    wh->GetWorker()->RecordAccessTiming(&wr);
...
int GAlloc::Write(const GAddr addr, const Size offset, void* buf,
  ...
  return count;
#else
  // Use async write + MFence to achieve Sequential Consistency
  // This allows accurate timing measurement of complete write operation
  WorkRequest wr { };
  wr.op = WRITE;
  // wr.flag = flag | ASYNC; // PSO
  wr.flag = flag; // TSO
  wr.size = count;
  wr.addr = GADD(addr, offset);
  wr.ptr = buf;
  ...
  // Insert MFence after each Write to achieve Sequential Consistency
  // This ensures the write completes before proceeding
  MFence();
  
  // Record access end time after MFence completes and collect timing stats
  wr.access_end_time = get_time();
  wh->GetWorker()->RecordAccessTiming(&wr);
