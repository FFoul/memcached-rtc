...
int WorkerHandle::SendRequest(WorkRequest* wr) {
  ...
  *notify_buf = 1;  //not useful to set it to 1 if boost_queue is enabled
  ...
  // Cache check end time should already be recorded in cache.cc (for remote access)
  // or in local_request_cache.cc (for local access)
  // If we reach here and cache_check_end_time is still 0, it means the timing logic is broken
  if (wr->access_start_time > 0 && wr->cache_check_end_time == 0) {
    epicLog(LOG_WARNING, "cache_check_end_time not recorded! This should not happen. is_cache_hit=%d, ret=%d, op=%d",
            wr->is_cache_hit_, ret, wr->op);
  }
...
void WorkerHandle::ReportCacheStatistics() {
    ...
    fprintf(stdout, "%lld\t%lld\t%lld\t%lld\t%lld\t%lld\t%lld\t%lld\t%lld\n"
                       ...
                       , worker->no_remote_writes_hit_.load() // remote + cache hit
...
void WorkerHandle::CollectCacheStatistics() {
  ...
  fprintf(stdout, "%lld\t%lld\t%lld\t%lld\t%lld\t%lld\t%lld\t%lld\t%lld\n"
      , worker->cache_read_hit_case1_.load() // cache is going to be invalid, but still usable for read 
      , worker->cache_read_hit_case2_.load() // cache is going from shared to dirty, but still usable for read 
      , worker->cache_read_hit_case3_.load() // in transition state while cache read/write
      , worker->cache_read_hit_case4_.load() // cache is going from shared to dirty, but still usable for read
      , worker->cache_write_hit_case1_.load() // cache is going to be invalid, but still usable for read
      , worker->cache_write_hit_case2_.load() // cache is going from shared to dirty, but still usable for read
      , worker->cache_write_hit_case3_.load() // in transition state while cache read/write
