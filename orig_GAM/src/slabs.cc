...
void* SlabAllocator::slabs_init(const size_t limit, const double factor,
  ...
  // Initialize TLSF mutex
  pthread_mutex_init(&tlsf_lock, NULL);
  
  // Allocate memory for TLSF
  mem_base = (char*) mmap_malloc(mem_limit);
  if (mem_base == NULL) {
    fprintf(stderr, "Error: Failed to allocate memory for TLSF\n");
    pthread_mutex_destroy(&tlsf_lock);
    return NULL;
  }

  printf("TLSF memory allocated at %p with size %ld bytes\n", mem_base, mem_limit);
  
  // Initialize TLSF with the allocated memory
  tlsf_handle = tlsf_create_with_pool(mem_base, mem_limit);
  if (tlsf_handle == NULL) {
    fprintf(stderr, "Error: Failed to create TLSF instance\n");
    mmap_free(mem_base);
    mem_base = NULL;
    pthread_mutex_destroy(&tlsf_lock);
    return NULL;
  ...
  epicLog(LOG_INFO, "TLSF allocator initialized with %ld bytes", mem_limit);
  
  return mem_base;
#else
  // Original SLAB implementation
...
#ifdef USE_TLSF
  // TLSF implementation
  if (mem_limit == 0 || tlsf_handle == NULL) {
    epicLog(LOG_WARNING, "TLSF allocator is not initialized");
    return NULL;
  }
  
  pthread_mutex_lock(&tlsf_lock);
  void* ret = tlsf_malloc(tlsf_handle, size);
  if (ret) {
    mem_free -= tlsf_block_size(ret);
  } else {
    epicLog(LOG_FATAL, "TLSF malloc failed: requested size = %zu bytes, available memory = %zu bytes", size, mem_free);
  }
...
void * SlabAllocator::sb_aligned_malloc(size_t size, size_t block) {
...
#ifdef USE_TLSF
  // TLSF implementation with alignment
  if (mem_limit == 0 || tlsf_handle == NULL) {
    epicLog(LOG_WARNING, "TLSF allocator is not initialized");
    return NULL;
  }
  ...
  pthread_mutex_lock(&tlsf_lock);
  void* ret = tlsf_memalign(tlsf_handle, block, size);
  if (ret) {
    mem_free -= tlsf_block_size(ret);
  } else {
    epicLog(LOG_FATAL, "TLSF memalign failed: requested size = %zu bytes, alignment = %zu, available memory = %zu bytes", size, block, mem_free);
  }
  pthread_mutex_unlock(&tlsf_lock);
  return ret;
#else
...
void * SlabAllocator::sb_calloc(size_t count, size_t size) {
  ...
  if (ptr != NULL) {
    ...
    epicLog(LOG_FATAL, "TLSF calloc failed: requested count = %zu, size = %zu bytes (total = %zu bytes)", count, size, count * size);
  }
  return ptr;
#else
  // Original SLAB implementation
...
void * SlabAllocator::sb_aligned_calloc(size_t count, size_t size,
  ...
  if (ptr != NULL) {
    ...
    epicLog(LOG_FATAL, "TLSF aligned_calloc failed: requested count = %zu, size = %zu bytes (total = %zu bytes), alignment = %zu", count, size, count * size, block);
  }
  return ptr;
#else
  // Original SLAB implementation
...
void *SlabAllocator::sb_realloc(void * ptr, size_t size) {
  ...
  void* ret = tlsf_realloc(tlsf_handle, ptr, size);
  if (ret) {
    size_t new_size = tlsf_block_size(ret);
    mem_free = mem_free + old_size - new_size;
  } else {
    epicLog(LOG_FATAL, "TLSF realloc failed: ptr = %p, old_size = %zu bytes, requested new_size = %zu bytes, available memory = %zu bytes", ptr, old_size, size, mem_free);
  }
  pthread_mutex_unlock(&tlsf_lock);
  return ret;
#else
