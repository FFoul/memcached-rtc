...
void Worker::ProcessPendingRead(Client* cli, WorkRequest* wr) {
  ...
  epicLog(LOG_INFO, "ProcessPendingRead: wr=%p, wr->id=%d, wr->request_send_time=%ld, wr->reply_receive_time=%ld", 
          wr, wr->id, wr->request_send_time, wr->reply_receive_time);
  
  // Record reply receive time (end of T2 - data received at request node)
  if (wr->request_send_time > 0 && wr->reply_receive_time == 0) {
    wr->reply_receive_time = get_time();
    epicLog(LOG_INFO, "ProcessPendingRead: Recorded reply_receive_time=%ld, home_processing_time=%ld",
            wr->reply_receive_time, wr->home_processing_time);
  }
  ...
  if (--parent->counter == 0) {  //read all the data
    // Copy timing information from child work request to parent only if parent doesn't have it
    // The parent tracks the overall access timing, child tracks sub-operation timing
    epicLog(LOG_INFO, "ProcessPendingRead: Checking timing copy from wr to parent, wr->request_send_time=%ld, parent->request_send_time=%ld", 
            wr->request_send_time, parent->request_send_time);
    
    // Only copy if parent hasn't recorded these times yet
    // This handles the case where the parent request creates child requests for fetching data
    if (wr->request_send_time > 0 && parent->request_send_time == 0) {
      parent->request_send_time = wr->request_send_time;
    }
    if (wr->request_receive_time > 0 && parent->request_receive_time == 0) {
      parent->request_receive_time = wr->request_receive_time;
    }
    if (wr->reply_send_time > 0 && parent->reply_send_time == 0) {
      parent->reply_send_time = wr->reply_send_time;
    }
    if (wr->reply_receive_time > 0 && parent->reply_receive_time == 0) {
      parent->reply_receive_time = wr->reply_receive_time;
    }
    if (wr->cache_check_end_time > 0 && parent->cache_check_end_time == 0) {
      parent->cache_check_end_time = wr->cache_check_end_time;
    }
    // Don't override parent's access_category if it's already set
    if (wr->access_category > 0 && parent->access_category == 0) {
      parent->access_category = wr->access_category;
    }
...
void Worker::ProcessPendingReadForward(Client* cli, WorkRequest* wr) {
  ...
  // Record end time and update statistics for INVALIDATE_FORWARD
  if (wr->invalidate_start_time > 0) {
    wr->invalidate_end_time = get_time();
    long duration = wr->invalidate_end_time - wr->invalidate_start_time;
    
    if (wr->invalidate_type == 2) { // INVALIDATE_FORWARD
      this->invalidate_forward_count_++;
      this->invalidate_forward_total_time_ += duration;
      
      epicLog(LOG_INFO, "INVALIDATE_TIMING: type=INVALIDATE_FORWARD, duration=%ld ns, start=%ld, end=%ld, addr=%lx", 
              duration, wr->invalidate_start_time, wr->invalidate_end_time, wr->addr);
    }
  }
...
void Worker::ProcessPendingWrite(Client* cli, WorkRequest* wr) {
  ...
  // Record reply receive time (end of T2 reply phase - permission/ACK received at request node)
  if (wr->reply_send_time > 0 && wr->reply_receive_time == 0) {
    wr->reply_receive_time = get_time();
  }
  ...
  if (!(wr->flag & REPEATED) && !(wr->flag & REQUEST_DONE)) {
    if (WID(wr->addr) == cli->GetWorkerId()) {  //from home node, Case 4
      ...
      // Record invalidate timing when counter reaches 0
      if (wr->counter == 0 && wr->invalidate_start_time > 0) {
        wr->invalidate_end_time = get_time();
        long duration = wr->invalidate_end_time - wr->invalidate_start_time;
        
        // Update statistics based on invalidate type
        switch (wr->invalidate_type) {
          case 1: // INVALIDATE
            this->invalidate_count_++;
            this->invalidate_total_time_ += duration;
            break;
          case 3: // FETCH_AND_INVALIDATE
            this->fetch_and_invalidate_count_++;
            this->fetch_and_invalidate_total_time_ += duration;
            break;
        }
        
        // Log the invalidate timing information
        const char* type_name = "";
        switch (wr->invalidate_type) {
          case 1: type_name = "INVALIDATE"; break;
          case 2: type_name = "INVALIDATE_FORWARD"; break;
          case 3: type_name = "FETCH_AND_INVALIDATE"; break;
          case 4: type_name = "WRITE_FORWARD"; break;
          case 5: type_name = "ACTIVE_INVALIDATE"; break;
          default: type_name = "UNKNOWN"; break;
        }
        
        epicLog(LOG_INFO, "INVALIDATE_TIMING: type=%s, duration=%ld ns, start=%ld, end=%ld, addr=%lx", 
                type_name, duration, wr->invalidate_start_time, wr->invalidate_end_time, wr->addr);
      }
    }
  }
  ...
  if (wr->status) {  //failed from one of the responders
    ...
    if (wr->counter == 0) {
      ...
      // Copy timing information from child work request to parent before notifying
      // This is crucial for collecting accurate statistics in gallocator.cc
      if (wr->request_send_time > 0) {
        parent->request_send_time = wr->request_send_time;
      }
      if (wr->request_receive_time > 0) {
        parent->request_receive_time = wr->request_receive_time;
      }
      if (wr->reply_send_time > 0) {
        parent->reply_send_time = wr->reply_send_time;
      }
      if (wr->reply_receive_time > 0) {
        parent->reply_receive_time = wr->reply_receive_time;
      }
      if (wr->cache_check_end_time > 0) {
        parent->cache_check_end_time = wr->cache_check_end_time;
      }
      if (wr->access_category > 0) {
        parent->access_category = wr->access_category;
      }
...
void Worker::ProcessPendingWriteForward(Client* cli, WorkRequest* wr) {
  ...
  // Record end time and update statistics for WRITE_FORWARD
  if (wr->invalidate_start_time > 0) {
    wr->invalidate_end_time = get_time();
    long duration = wr->invalidate_end_time - wr->invalidate_start_time;
    
    if (wr->invalidate_type == 4) { // WRITE_FORWARD
      this->write_forward_count_++;
      this->write_forward_total_time_ += duration;
      
      epicLog(LOG_INFO, "INVALIDATE_TIMING: type=WRITE_FORWARD, duration=%ld ns, start=%ld, end=%ld, addr=%lx", 
              duration, wr->invalidate_start_time, wr->invalidate_end_time, wr->addr);
    }
  }
...
void Worker::ProcessPendingInvalidateForward(Client* cli, WorkRequest* wr) {
  ...
  if (wr->counter == 0) {
    ...
    if (WRITE == parent->op) {
      access_type = remote_write_shared;
...
void Worker::ProcessPendingRequest(Client* cli, WorkRequest* wr) {
  ...
  switch (wr->op) {
    ...
    case FETCH_AND_SHARED: {
      // printf("ProcessPendingRead\n");
      ProcessPendingRead(cli, wr);
      break;
    }
    case READ_FORWARD: {
      // printf("ProcessPendingReadForward\n");
    ...
    case WRITE_PERMISSION_ONLY: {
      // printf("ProcessPendingWrite\n");
      ProcessPendingWrite(cli, wr);
      break;
    }
    case WRITE_FORWARD:  //Case 4 in home node
    {
      access_type = remote_write_dirty;
      // printf("ProcessPendingWriteForward\n");
      ProcessPendingWriteForward(cli, wr);
      break;
    }
    case WRITE_BACK: {
      // printf("ProcessPendingWriteBack\n");
      ProcessPendingEvictDirty(cli, wr);
      break;
    }
    case INVALIDATE_FORWARD: {
      // printf("ProcessPendingInvalidateForward\n");
