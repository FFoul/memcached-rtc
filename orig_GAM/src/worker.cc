...
#endif
{
  ...
  // Initialize invalidate statistics
  invalidate_count_ = 0;
  invalidate_total_time_ = 0;
  invalidate_forward_count_ = 0;
  invalidate_forward_total_time_ = 0;
  fetch_and_invalidate_count_ = 0;
  fetch_and_invalidate_total_time_ = 0;
  write_forward_count_ = 0;
  write_forward_total_time_ = 0;
...
void Worker::StartService(Worker* w) {
  ...
  while (likely(!eventLoop->stop)) {
    ...
    for (int i = 0; i < ne; ++i) {
      ...
      w->ProcessRdmaRequest_total_time += (end_time - start_time);
#endif
    }
#else
...
unsigned long long Worker::SubmitRequest(Client* cli, WorkRequest* wr, int flag,
  ...
  // Record request send time when sending remote requests
  // This marks the start of T2 request phase and the end of T1 (cache check phase)
  // printf("SubmitRequest: flag=%d, REQUEST_SEND=%d, access_start_time=%ld, request_send_time=%ld, op=%d\n",
  //         flag, (flag & REQUEST_SEND), wr->access_start_time, wr->request_send_time, wr->op);
  if ((flag & REQUEST_SEND) && wr->access_start_time > 0 && wr->request_send_time == 0) {
    // End T1 here for cache miss cases (before sending remote request)
    if (wr->cache_check_end_time == 0) {
      wr->cache_check_end_time = get_time();
    }
    wr->request_send_time = get_time();
    // printf("SubmitRequest: Set request_send_time=%ld for wr=%p, wr->id=%d, addr=%lx\n", 
    //         wr->request_send_time, wr, wr->id, wr->addr);
  }
...
void Worker::PrintInvalidateStatistics() {
  epicLog(LOG_FATAL, "=== INVALIDATE STATISTICS ===");
  epicLog(LOG_FATAL, "INVALIDATE: count=%lu, total_time=%lu ns, avg_time=%.2f ns", 
          invalidate_count_.load(), invalidate_total_time_.load(),
          invalidate_count_.load() > 0 ? (double)invalidate_total_time_.load() / invalidate_count_.load() : 0.0);
  epicLog(LOG_FATAL, "INVALIDATE_FORWARD: count=%lu, total_time=%lu ns, avg_time=%.2f ns", 
          invalidate_forward_count_.load(), invalidate_forward_total_time_.load(),
          invalidate_forward_count_.load() > 0 ? (double)invalidate_forward_total_time_.load() / invalidate_forward_count_.load() : 0.0);
  epicLog(LOG_FATAL, "FETCH_AND_INVALIDATE: count=%lu, total_time=%lu ns, avg_time=%.2f ns", 
          fetch_and_invalidate_count_.load(), fetch_and_invalidate_total_time_.load(),
          fetch_and_invalidate_count_.load() > 0 ? (double)fetch_and_invalidate_total_time_.load() / fetch_and_invalidate_count_.load() : 0.0);
  epicLog(LOG_FATAL, "WRITE_FORWARD: count=%lu, total_time=%lu ns, avg_time=%.2f ns", 
          write_forward_count_.load(), write_forward_total_time_.load(),
          write_forward_count_.load() > 0 ? (double)write_forward_total_time_.load() / write_forward_count_.load() : 0.0);
  
  uint64_t total_count = invalidate_count_.load() + invalidate_forward_count_.load() + 
                         fetch_and_invalidate_count_.load() + write_forward_count_.load();
  uint64_t total_time = invalidate_total_time_.load() + invalidate_forward_total_time_.load() + 
                        fetch_and_invalidate_total_time_.load() + write_forward_total_time_.load();
  epicLog(LOG_FATAL, "TOTAL: count=%lu, total_time=%lu ns, avg_time=%.2f ns", 
          total_count, total_time, total_count > 0 ? (double)total_time / total_count : 0.0);
}
