  ...
  for (GAddr i = start_blk; i < end;) {
    ...
    if ((cline = GetCLine(i))) {
      ...
      if (unlikely(InTransitionState(state))) {
        ...
        // Record cache check end time for cache hit (transition state case)
      ...
      if (READ == wr->op) {
        // Record cache check end time for cache hit (READ)
        if (wr->access_start_time > 0 && wr->cache_check_end_time == 0) {
          wr->cache_check_end_time = get_time();
        }
#ifdef SELECTIVE_CACHING
        ...
        // Record cache check end time (WRITE - will determine hit/miss below)
        if (wr->access_start_time > 0 && wr->cache_check_end_time == 0) {
          wr->cache_check_end_time = get_time();
        }
#ifdef SELECTIVE_CACHING
...
//					}
          ...
          if (wr->flag & GFUNC) {
            ...
            worker->access_type = worker->remote_write_cache_hit;
#endif
            epicAssert(len);
      ...
      // Record cache check end time for cache miss
      if (wr->access_start_time > 0 && wr->cache_check_end_time == 0) {
        wr->cache_check_end_time = get_time();
      }
...
void Cache::Evict(CacheLine* cline) {
  ...
  if (CACHE_SHARED == state) {
    ...
    // Record invalidate start time and type for ACTIVE_INVALIDATE
    wr->invalidate_start_time = get_time();
    wr->invalidate_type = 5; // ACTIVE_INVALIDATE
