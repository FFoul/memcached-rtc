...
// hjx: add a function to check whether the device is active
inline bool checkActiveResources(ibv_device *dev) {
  // get context
  ibv_context *context = ibv_open_device(dev);
  if (!context) {
    epicLog(LOG_DEBUG, "Failed to open device %s\n", ibv_get_device_name(dev));
    return false;
  }
  ...
  if (ibv_query_port(context, 1, &port_attr)) {
    epicLog(LOG_DEBUG, "Failed to query port status for device %s\n", ibv_get_device_name(dev));
  ...
  if (port_attr.state != IBV_PORT_ACTIVE) {
    epicLog(LOG_DEBUG, "Device %s is not active (port state: %d)\n", ibv_get_device_name(dev), port_attr.state);
    ibv_close_device(context);
    return false;
  }

  epicLog(LOG_DEBUG, "Device %s is active\n", ibv_get_device_name(dev));
  return true;

}

// hjx: check whether the device is active before creating a new RdmaResource
  ...
  if (devName) {
    for (std::vector<RdmaResource *>::iterator it = resources.begin();
      ...
      if (!strcmp((*it)->GetDevname(), devName)
          && (*it)->IsMaster() == isMaster && checkActiveResources((*it)->GetDevice()))
        return (*it);
    }
  }
...
const char* RdmaContext::GetRdmaConnString() {
  ...
  for (int i = 0; dev_list[i] != NULL; ++i) {
      dev = dev_list[i];
      if (dev == this->resource->context->device) {
          epicLog(LOG_DEBUG, "Using device: %s", ibv_get_device_name(dev));
      }
  }
  ...
  // Check port status before querying GID
  struct ibv_port_attr port_attr;
  int rc_port = ibv_query_port(this->resource->context, this->resource->ibport, &port_attr);
  if (rc_port == 0) {
      if (port_attr.state == IBV_PORT_ACTIVE) {
          epicLog(LOG_DEBUG, "Device port %d is ACTIVE", this->resource->ibport);
      } else {
          epicLog(LOG_DEBUG, "Device port %d is not ACTIVE (state: %d)", this->resource->ibport, port_attr.state);
      }
  } else {
      epicLog(LOG_DEBUG, "Failed to query port status for port %d", this->resource->ibport);
  }
  ...
  if (rc == 0) {
    epicLog(LOG_DEBUG, "Successfully queried GID at index 3: ");
    for (int i = 0; i < 16; ++i) {
        epicLog(LOG_DEBUG, "%02x ", ((unsigned char*)&(this->resource->gid))[i]);
    }
  } else {
      epicLog(LOG_DEBUG, "Failed to query GID at index 3");
  }
...
ssize_t RdmaContext::Rdma(ibv_wr_opcode op, const void* src, size_t len,
  ...
  if (pending_msg >= max_pending_msg) {
    ...
    // epicLog(LOG_FATAL, "pending_msg = %d, max_pending_msg = %d\n", pending_msg.load(), max_pending_msg);
    
    // Record RDMA call statistics even when added to pending queue
    if (op == IBV_WR_SEND) {
      RecordRdmaCall(0);  // Send
    } else if (op == IBV_WR_RDMA_WRITE) {
      RecordRdmaCall(1);  // Write
    } else if (op == IBV_WR_RDMA_WRITE_WITH_IMM) {
      RecordRdmaCall(2);  // Write with Imm
    } else if (op == IBV_WR_RDMA_READ) {
      RecordRdmaCall(3);  // Read
    } else if (op == IBV_WR_ATOMIC_CMP_AND_SWP) {
      RecordRdmaCall(4);  // CAS
    }
  ...
  if (op == IBV_WR_SEND) {
    ...
    epicLog(LOG_FATAL, "unsupported RDMA OP");
    return -1;
  }
