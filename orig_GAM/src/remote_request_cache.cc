...
void Worker::ProcessRemoteRead(Client* client, WorkRequest* wr) {
  // Record request receive time at home node (end of T2 request phase)
  // printf("ProcessRemoteRead: wr=%p, wr->id=%d, wr->request_send_time=%ld, wr->request_receive_time=%ld\n", 
  //        wr, wr->id, wr->request_send_time, wr->request_receive_time);
  if (wr->request_send_time > 0 && wr->request_receive_time == 0) {
    wr->request_receive_time = get_time();
    // printf("ProcessRemoteRead: Set request_receive_time=%ld for wr=%p, wr->id=%d, addr=%lx\n", 
    //        wr->request_receive_time,  wr, wr->id, wr->addr);
  }
  ...
  if (directory.GetState(entry) != DIR_DIRTY) {  //it is shared or exclusively owned (Case 2)
    ...
    if (directory.IsBlockWLocked(entry)) {
      if (wr->flag & TRY_LOCK) {  //reply directly with lock failed
        ...
        wr->op = READ_REPLY;
        // Record reply send time before sending error reply
        if (wr->request_receive_time > 0 && wr->reply_send_time == 0) {
          wr->reply_send_time = get_time();
        }
        directory.unlock(laddr);
    ...
    // Record reply send time and calculate home node processing time
    // This is cache miss simple: home node has the data, no forward needed
    if (wr->request_receive_time > 0 && wr->reply_send_time == 0) {
      wr->reply_send_time = get_time();
      // Calculate processing time on home node (using local clock, so it's accurate)
      wr->home_processing_time = wr->reply_send_time - wr->request_receive_time;
    }
    // Mark as cache miss simple if this is a cache miss access
    if (wr->access_category == 2) {
      wr->access_category = 2;  // CACHE_MISS_SIMPLE: direct fetch from home node
    }
    ...
    // DIR_DIRTY: need to forward to owner, this is cache miss complex
    epicAssert(!directory.IsBlockLocked(entry));
    // Mark as cache miss complex if this is a cache miss access
    if (wr->access_category == 2) {
      wr->access_category = 3;  // CACHE_MISS_COMPLEX: requires forward operation
    }
...
void Worker::ProcessRemoteReadReply(Client* client, WorkRequest* wr) {
  ...
  // Copy timing information from READ_REPLY message to pending work
  if (wr->request_receive_time > 0) {
    pwr->request_receive_time = wr->request_receive_time;
  }
  if (wr->reply_send_time > 0) {
    pwr->reply_send_time = wr->reply_send_time;
  }
  // Record reply receive time at request node
  if (pwr->request_send_time > 0 && pwr->reply_receive_time == 0) {
    pwr->reply_receive_time = get_time();
  }
...
void Worker::ProcessRemoteWrite(Client* client, WorkRequest* wr) {
  // Record request receive time at home node (end of T2 request phase)
  // printf("ProcessRemoteWrite: wr=%p, wr->id=%d, wr->request_send_time=%ld, wr->request_receive_time=%ld\n", 
  //        wr, wr->id, wr->request_send_time, wr->request_receive_time);
  if (wr->request_send_time > 0 && wr->request_receive_time == 0) {
    // printf("ProcessRemoteWrite: Set request_receive_time=%ld for wr=%p, wr->id=%d, addr=%lx\n", 
    //        get_time(), wr, wr->id, wr->addr);
    wr->request_receive_time = get_time();
  }
  ...
  directory.lock(laddr);
  long d_start = get_time();
  DirEntry* entry = directory.GetEntry(laddr);
  long d_end = get_time();
  // printf("directory check time = %ld\n", d_end - d_start);
  ...
  if (state != DIR_DIRTY) {
    ...
    if (directory.IsBlockLocked(entry)) {
      ...
      if (wr->flag & TRY_LOCK) {  //reply directly with lock failed
        ...
        wr->op = WRITE_REPLY;
        wr->counter = 0;
        // Record reply send time before sending reply
        if (wr->request_receive_time > 0 && wr->reply_send_time == 0) {
          wr->reply_send_time = get_time();
        }
        SubmitRequest(client, wr);
    ...
    if (state == DIR_SHARED) {
      // DIR_SHARED: need to invalidate sharers, this is cache miss complex
      // Mark as cache miss complex if this is a cache miss access
      if (wr->access_category == 2) {
        wr->access_category = 3;  // CACHE_MISS_COMPLEX: requires invalidate operation
      }
      ...
      // Mark as cache miss complex if this is a cache miss access (needs invalidation)
      if (wr->access_category == 2) {
        wr->access_category = 3;  // CACHE_MISS_COMPLEX: requires invalidate operation
      }
      
      // Record invalidate start time and type
      lwr->invalidate_start_time = get_time();
      lwr->invalidate_type = 2; // INVALIDATE_FORWARD=2
      
      bool first = true;
      for (auto it = shared.begin(); it != shared.end(); it++) {
        Client* cli = GetClient(*it);
        if (cli == client) {
          // cache hit but cache state = shared
        ...
        SubmitRequest(cli, lwr); // send invalidation
        //lwr->counter++;
      }
      ...
      // DIR_UNSHARED: can write directly, this is cache miss simple
      // Mark as cache miss simple if this is a cache miss access
      if (wr->access_category == 2) {
        wr->access_category = 2;  // CACHE_MISS_SIMPLE: direct write to home node
      }
    ...
    // Record invalidate start time and type
    lwr->invalidate_start_time = get_time();
    lwr->invalidate_type = 4; // WRITE_FORWARD=4
...
void Worker::ProcessRemoteWriteReply(Client* client, WorkRequest* wr) {
  ...
  // Copy timing information from WRITE_REPLY message to pending work
  if (wr->request_receive_time > 0) {
    pwr->request_receive_time = wr->request_receive_time;
  }
  if (wr->reply_send_time > 0) {
