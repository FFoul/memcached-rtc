...
class Worker : public Server {
  ...
  // Access timing collector for performance analysis
  AccessTimingCollector access_timing_collector_;
 ...
 public:
  ...
  // Invalidate timing statistics
  atomic<Size> invalidate_count_;
  atomic<Size> invalidate_total_time_;
  atomic<Size> invalidate_forward_count_;
  atomic<Size> invalidate_forward_total_time_;
  atomic<Size> fetch_and_invalidate_count_;
  atomic<Size> fetch_and_invalidate_total_time_;
  atomic<Size> write_forward_count_;
  atomic<Size> write_forward_total_time_;
  
  // Function to print invalidate statistics
  void PrintInvalidateStatistics();

  enum {
    local_write_unshared = 0,
    local_write_shared = 1,
    local_write_dirty = 2,
    remote_write_cache_hit = 3,
    remote_write_unshared = 4,
    remote_write_shared = 5,
    remote_write_dirty = 6,
    false_oh_no = 7
  };

  atomic<Size> access_type;
  ...
  // Remote thread processing functions
  void ProcessRemoteThreadCreate(Client* client, WorkRequest* wr);
  void ProcessRemoteThreadJoin(Client* client, WorkRequest* wr);
  ...
  // Access timing collector methods
  inline void RecordAccessTiming(WorkRequest* wr) {
    if (access_timing_collector_.IsEnabled()) {
      access_timing_collector_.RecordAccess(wr);
    }
  }
  
  inline void EnableAccessTimingCollector() {
    access_timing_collector_.Init(GetWorkerId());
  }
  
  inline void DisableAccessTimingCollector() {
    access_timing_collector_.SetEnabled(false);
  }
  
  inline void PrintAccessTimingStats() {
    access_timing_collector_.PrintStats();
  }
  
  inline void ResetAccessTimingStats() {
    access_timing_collector_.ResetStats();
  }
