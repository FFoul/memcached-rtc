...
class GAllocFactory {
  ...
  // Server configuration for distributed threading
  static int total_servers;
  static int current_server_id;
  // local worker pointer for GetLocalWorker
  static Worker* local_worker;
 public:
#ifdef GFUNC_SUPPORT
#define MAX_GFUNCS 100
  ...
  // Server configuration management for distributed threading
  static void SetServerConfig(int total, int current_id) {
    lock.lock();
    total_servers = total;
  ...
  static int GetTotalServers() {
    // No lock needed for read-only access after initialization
    return total_servers;
  }

  static int GetCurrentServerId() {
    // No lock needed for read-only access after initialization
    return current_server_id;
  }

  // Global resource access for distributed threading
  static Worker* GetWorker() {
    // No lock needed for read-only access after initialization
    return worker;
  }

  static void setLocalWorker(Worker* localworker) {
