...
void item_lock(uint32_t hv) { mutex_lock(&item_locks[hv & item_lock_mask]); }
...
#ifdef RTC_BENCHMARK
  STATS_LOCK();
  stats.accepting_conns = do_accept;
  STATS_UNLOCK();
#else
 ...
 */
#ifdef RTC_BENCHMARK
enum delta_result_type add_delta(conn *c, const uint64_t key, int incr,
...
LIBEVENT_THREAD *rtc_thread_get(int thread_id) {
  if (thread_id < 0 || thread_id >= settings.num_threads || threads == NULL) {
    return NULL;
  }
