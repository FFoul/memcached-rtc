 ...
 */
int mt_is_listen_thread() { return pthread_self() == threads[0].thread_id; }
 ...
 */
item *mt_item_alloc(char *key, size_t nkey, int flags, rel_time_t exptime,
                    int nbytes) {
  item *it;
  pthread_mutex_lock(&cache_lock);
 ...
 */
item *mt_item_get_notedeleted(const char *key, const size_t nkey,
                              bool *delete_locked) {
  item *it;
  pthread_mutex_lock(&cache_lock);
 ...
 */
char *mt_item_cachedump(unsigned int slabs_clsid, unsigned int limit,
                        unsigned int *bytes) {
  char *ret;
...
void mt_stats_lock() { pthread_mutex_lock(&stats_lock); }

void mt_stats_unlock() { pthread_mutex_unlock(&stats_lock); }
